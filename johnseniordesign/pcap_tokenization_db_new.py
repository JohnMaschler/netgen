...
class MultiPacketStreamDatasetDB(Dataset):
    def __init__(self, db_path="traffic_data.db", max_packets_per_flow=None, num_prompts_per_flow=4, max_flows=None):
        ...
        self.max_flows = max_flows  # New parameter to cap the number of flows
        self.samples = []
        self._load_data()
    ...
    def _load_data(self):
        ...
        for flow_row in flows:
            ...
            additional_prompts = [
                ...
                f"Create a packet sequence that would occur when {base_prompt}"
            ]
            
            all_prompts = prompts + additional_prompts
            
            selected_prompts = random.sample(all_prompts, min(self.num_prompts_per_flow, len(all_prompts)))
