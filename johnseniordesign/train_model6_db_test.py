...
def main():
    ...
    text_tokenizer = SentencePieceTokenizer(model_path="my_spm_new.model")
    ...
    # Comment out this entire checkpoint loading section
    """
    ...
    if os.path.exists(checkpoint_dir):
        ...
        if checkpoints:
            ...
            if 'text_tokenizer' in checkpoint and 'packet_tokenizer' in checkpoint:
                text_tokenizer.word2id = checkpoint['text_tokenizer']
                text_tokenizer.id2word = {v: k for k, v in text_tokenizer.word2id.items()}
                text_tokenizer.vocab_size = len(text_tokenizer.word2id)
                
                packet_tokenizer.token2id = checkpoint['packet_tokenizer']
                packet_tokenizer.id2token = {v: k for k, v in packet_tokenizer.token2id.items()}
                packet_tokenizer.vocab_size = len(packet_tokenizer.token2id)
                
                print("Loaded tokenizers from checkpoint")
    """
    
    # Now create the model with the correct vocabulary sizes
