...
def train_transformer(
    ...
    for epoch in range(start_epoch, num_epochs + start_epoch):
        ...
        total_epochs += 1
        print(f"Epoch [{total_epochs}/{start_epoch + num_epochs}], Loss: {avg_loss:.4f}, Time: {epoch_time_minutes}mins, {epoch_time_seconds}sec")
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save({
                'epoch': epoch + 1,  # Save the next epoch number to resume from here
    ...
    plt.plot(range(start_epoch + 1, num_epochs + 1), losses)
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
...
def main():
    ...
    for pkt_tokens in packet_data:
        flat_packet_data.extend(pkt_tokens)
    ...
    # Check if a checkpoint exists and resume training from there if available
    total_epochs = 100
    start_epoch = 0
    if os.path.exists('model3.pth'):
        checkpoint = torch.load('model3.pth', map_location=device)

        checkpoint_vocab_size = checkpoint['model_state_dict']['encoder.word_embedding.weight'].shape[0]
        current_vocab_size = model.encoder.word_embedding.weight.shape[0]

        if checkpoint_vocab_size != current_vocab_size:
            print(f"Resizing embeddings: checkpoint vocab size = {checkpoint_vocab_size}, current vocab size = {current_vocab_size}")
            old_embeddings = checkpoint['model_state_dict']['encoder.word_embedding.weight']
            if current_vocab_size > checkpoint_vocab_size:
                
                new_embedding_weight = torch.cat([
                    old_embeddings,
                    torch.randn((current_vocab_size - checkpoint_vocab_size, old_embeddings.shape[1]), device=device)
                ], dim=0)
            else:
                # If current model has fewer embeddings, trim the checkpoint embeddings
                new_embedding_weight = old_embeddings[:current_vocab_size]
                # Update the checkpoint state dict with the resized embeddings
                checkpoint['model_state_dict']['encoder.word_embedding.weight'] = new_embedding_weight

        # Now load the modified state dict
        model.load_state_dict(checkpoint['model_state_dict'], strict=False)
        print("Resumed training from checkpoint with adjusted embeddings.")

        start_epoch = checkpoint.get('epoch', 0)
        total_epochs = checkpoint.get('total_epochs', 0)
        print(f"---- Resuming training from epoch {start_epoch}, total epochs trained: {total_epochs} ----")
    else:
        print("---- Starting training from scratch ----")


    total_epochs = train_transformer(
        model=model,
